...
export default function PostSummary({ summary }) {
  return (
    <div className='flex flex-col lg:items-end w-full lg:sticky lg:top-24 font-bebas underline-offset-2'>
      <div className='w-full flex flex-col bg-gray-800 p-4 rounded-xl gap-1'>
        <h4>Summary</h4>
        {summary.map((entry, i) => (
          <a
            key={entry._key}
            href={`#${i === 0 ? 'top' : getHeadingId(entry.children.map((c) => c.text))}`}
            className={clsx(
              entry.style === 'h1' && 'text-summary-1 underline',
              entry.style === 'h2' && 'pl-2 text-summary-2 underline',
