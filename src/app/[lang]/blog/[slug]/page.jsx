  ...
  return (
    <>
      ...
      <PageComponent page={post}>
        <div className='text-left gap-4 px-4 py-8 xl:px-16 w-full flex flex-col lg:flex-row relative bg-black max-w-full'>
          <div className='flex-1'>
            <PostSummary title={post.title} summary={summary}></PostSummary>
          </div>
          <div className='lg:flex-[3] rounded-xl bg-gray-800 p-8'>
            <h1
              id={encodeURIComponent(post.title)}
              className='text-5xl text-left w-full'
            >
              {post.title}
            </h1>
            {post.body.map((item) => (
